...
def main():
    ...
    try:
        ...
        if state == 'present':
            ...
            if nexthop_type == 'next-vr':
                if not existing_route:
                    xpath = ("/config/devices/entry[@name='localhost.localdomain']/network/virtual-router/entry"
                            "[@name='%s']/routing-table/ip/static-route/entry[@name='%s']" % (virtual_router,name))
                    element = ('<entry name="%s"><path-monitor><enable>no</enable><failure-condition>'
                            'any</failure-condition><hold-time>2</hold-time></path-monitor><nexthop>'
                            '<next-vr>%s</next-vr></nexthop><bfd><profile>None</profile></bfd><metric>10</metric>'
                            '<destination>%s</destination><route-table><unicast/></route-table></entry>' % (name,next_vr,destination))
                    device.xapi.edit(xpath=xpath,element=element)
                    changed = True
            else:
