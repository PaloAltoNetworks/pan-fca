resource "panos_panorama_nat_rule_group" "pano_source_nat_dipp" {
  count        = "${var.device_group != "" && var.rule_type == "source_nat_dipp_static" ? 1 : 0}"
...
resource "panos_panorama_nat_rule_group" "pano_source_dest_nat_dipp_dynamic" {
  count        = "${var.device_group != "" && var.rule_type == "source_nat_dipp_dynamic" ? 1 : 0}"
  device_group = "${var.device_group}"
  rulebase     = "${var.rulebase_type}"
  ...
  rule {
    ...
    translated_packet {
      ...
      destination {
        dynamic {
          address = "${var.destination_tp_dynamic_address}"
          port    = "${var.destination_tp_port}"
        }
      }
    }
...
resource "panos_nat_rule_group" "fw_source_nat_dipp_static" {
  count        = "${var.device_group == "" && var.rule_type == "source_nat_dipp_static" ? 1 : 0}"
  device_group = "${var.device_group}"
  rulebase     = "${var.rulebase_type}"
...
resource "panos_nat_rule_group" "fw_source_nat_dipp_dynamic" {
  count        = "${var.device_group == "" && var.rule_type == "source_nat_dipp_dynamic" ? 1 : 0}"
  device_group = "${var.device_group}"
  rulebase     = "${var.rulebase_type}"
