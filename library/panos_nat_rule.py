...
def main():
    ...
    dnat_type = module.params['dnat_type']
    ...
    # dnat_xpath = ("/config/devices/entry[@name='localhost.localdomain']/vsys/entry[@name='vsys1']" \
    #              "/rulebase/nat/rules/entry[@name='%s']" % (rule_name))
    # dnat_e = ('<entry name="%s"><dynamic-destination-translation><translated-address>%s</translated-address>'
    #           '</dynamic-destination-translation></entry>' % (rule_name, dnat_address))
    ...
    if state == 'present':
        ...
        if match is not None:
            if not match.equal(new_rule):
                try:
                    if dnat_type == "dynamic":
                        new_rule.apply()
                        #con.xapi.edit(xpath=dnat_xpath, element=dnat_e)
                    else:
                        new_rule.apply()
                except PanDeviceError as e:
